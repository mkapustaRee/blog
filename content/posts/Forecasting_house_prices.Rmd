---
title: "Predicting property prices using alternative data sources"
author: "Michal Kapusta"
date: "8 August 2018"
output: html_document
categories: ["R"]
tags: ["Webscrape", "LIME", "Alternative Data","Forecasting"]
---
...
df_ads <-  read_csv(file = "_files/list_of_ads [2018-08-08].csv")  %>% filter(!str_detect(street,pattern = "Bratislava")) 
urls <-  read_csv(file = "_files/list_of_urls [2018-08-08].csv") %>% pull(value)
gps_street <-  read_csv(file = "_files/street_with_gps.csv")  %>% drop_na() %>% rename(street_long = street)
street <- gps_street$street_long %>% str_split(", ",simplify = T) %>% as.data.frame() %>% pull(V1) 
gps_street$street <- street
gps_street <- gps_street %>% mutate(street = as.character(street))

df_ads_wGPS <- df_ads %>% left_join(gps_street %>% select(street,lon,lat))
...
The aim of this blog post is to showcase the usage of alternative data in order to improve predictive model. In this blogpost, I will be scraping data from property listing website in order to access real world data. After scraping and data cleaning steps I will add geospatial data (www.openstreetmaps.com) to enrich the database. My thesys is that the location features should improve the predictive models. Lets find out!
...
Using the package **rvest** we can access the information displayed on the webpage. The following datapoints are of our interest: 
...
The table shows we have suscesfully scraped 2379 records. Approximately 100 records are incomplete and will be removed from the analysis. Few other observations: 

* Average price for flat in Bratislava is ca **173.000 Eur**
* Average size of the flat is **72.7** square meters
* Average price per square meter is **2455** Eur
...
df_ads  %>% drop_na() %>% 
        ...
        labs(title = "DISTRIBUTION OF APPARTMENT PRICES IN BRATISLAVA")

```

The charts show that the majority of ads are price in range between 125.000 EUR - 200.000 EUR. The distribution is left-skewed an needs to be transformed using logarithm. The log10(price) ensures the distribution is normalized (more suitable for forecasting and readable). For backward conversion use calculate for example: 5.5^10 = 316227 EUR. 
...
* The most comeptitive priced is BA V. (small range between 25-75 % Quartile)  
...
* Price increases by number of bedrooms (exeption: BA V prices for 2 & 3 bedroom)
...
The chart above revelas that the flat prices are in direct relationship with the flat area. This is to be expected. Also Bratislava IV looks clustered around the model line - suggesting stronger relationship in the market. Is this a mere coincidence or hidden insight? 

Next, lets fit a linear model using data at we assembled so far. We  predict the price for the appartmetn using the traditional set of variables data scientists usually have at hand (mostly appartment metric and basic location variables)
...
model_recipe <- recipe(price ~ .  , data = df_train ) %>% 
                ...
                step_log(price, base = 10) %>%                  # log-transformation to normalize
...
The analysis so far used only several variables to analyse price of appartments in Bratislava. Compared to other datasets this is low. Kaggle competitions usually have dozens of metrics to build model. This metrics are usually covering data like: floor number, date of completition or fit-out quality. This additional data will ultimatively lead to more accurate model. Since we dont have such informations we look to **alternative data sources** to augment our database with non-standart metrics. Our thesys revolves around old real estate saying: **Location,Location,Location**.

In the following chapter we will:

* source open-street-map data  
* use location features to enrich the database 
* calculate distance from appartments to location features 
* use the results in the predictive model  

### Source the data using Open-Street-Map

The geospatial databases are a great source of information. The two major sources are **Google Maps API** or the open-source option **Open-street Map API**. The option from google supports location feature extraction, but it cost money to retrive thousends of records. 

First step is to enrich our database with geospatial data (longitute, latitude). For this purposes we will use the street adress and city information. Our dataset contains the street name and flats are located in Bratislava.   
...
### Attach the dataset to the appartment database
